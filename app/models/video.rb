class Video  < ActiveRecord::Base
  ...
  def write_to_mogile_fs
    (video_file_name,file_name_l,file_name_m,file_name_s) = Video.make_temp_file_name(self.id)
    if MogileFsUtil.put_file_to_fs(video_file_name, "/" +self.id.to_s, MOGILEFS_CLASS_VIDEOS)
      `rm -f #{video_file_name}`
    end
    if MogileFsUtil.put_file_to_fs(file_name_l, "/" + self.id.to_s + "_l", MOGILEFS_CLASS_VIDEOS)
      `rm -f #{file_name_l}`
    end
    if MogileFsUtil.put_file_to_fs(file_name_m, "/" + self.id.to_s + "_m", MOGILEFS_CLASS_VIDEOS)
      `rm -f #{file_name_m}`
    end
    if MogileFsUtil.put_file_to_fs(file_name_s, "/" + self.id.to_s + "_s", MOGILEFS_CLASS_VIDEOS)
      `rm -f #{file_name_s}`
    end
  end
